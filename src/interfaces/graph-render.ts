...
export interface RenderOptions {
  ...
  liveUrl?: string;
  pollMs?: number;
  /** Publish mode: strip node detail text (raw fact content) so only
   * structure — labels, edges, reasons, agents — leaves the machine. */
  redacted?: boolean;
}
...
function payload(graph: ContextGraph, redacted?: boolean): string {
  const out = redacted
    ? {
        ...graph,
        nodes: graph.nodes.map((n) => ({ ...n, detail: undefined })),
      }
    : graph;
...
var LIVE = ${live};
var POLL = ${pollMs};
var graph = ${payload(graph, meta.redacted)};
